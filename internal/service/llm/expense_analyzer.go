...
func buildAnalysisPrompt(expenses []*database.Expense, lobby *database.Lobby, analysisType string) string {
	...
	for _, exp := range expenses {
		...
		// Payment method aggregation (using ID as key since we don't have service access here)
		// The LLM will work with the data structure we provide
		if exp.PaymentMethodID.Valid {
			pmKey := fmt.Sprintf("payment_method_%d", exp.PaymentMethodID.Int64)
			paymentMethodTotals[pmKey] += exp.Amount
		}
	...
	if len(categoryTotals) > 0 {
		prompt.WriteString("- Category totals:\n")
		for cat, amt := range categoryTotals {
			prompt.WriteString(fmt.Sprintf("  %s: %.2f\n", cat, amt))
		}
	}
	...
	for _, exp := range expenses {
		...
		prompt.WriteString(fmt.Sprintf("%s (%s): $%.2f - %s [%s] - %s\n",
			exp.ExpenseDate.Format("2006-01-02"), timeRef, exp.Amount, desc, cat, spender))
		count++
	}
